...
try {
    ...
    // ユーザーの現在のhpとpointを取得
    $sql = 'SELECT gold, hp, point, hppoint FROM status2 WHERE userID = :userID';
    ...
    if ($result) {
        // gold -100, hp +25, point -2, hppoint +1
        if($result['gold']<100){
        ...
        // 更新処理
        $sql = 'UPDATE status2 SET gold = :gold, hp = :hp, point = :point, hppoint = :hppoint WHERE userID = :userID';
        $stmt = $dbh->prepare($sql);
