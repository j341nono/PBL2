...
try {
    ...
    // ユーザーの現在のhpとpointを取得
    $sql = 'SELECT gold, hp, power, point, hppoint, powerpoint FROM status WHERE userID = :userID';
    ...
    if ($result) {
        // gold -200, hp=75, power=10, point=14, hppoint=powerpoint=0
        if($result['gold']<200){
        ...
        // 更新処理
        $sql = 'UPDATE status SET gold = :gold, hp = :hp, power = :power, point = :point, hppoint = :hppoint, powerpoint = :powerpoint WHERE userID = :userID';
        $stmt = $dbh->prepare($sql);
