...
try {
    ...
    $pdo->beginTransaction();
      // ユーザーの現在のゴールドを取得
    $sql = "SELECT gold FROM status WHERE userID = :userID";
    ...
    // 完了タスクのゴールドを計算
    $totalGold = 0;
    $sql = "SELECT addgold FROM UserTasks WHERE userID = :userID AND taskID = :taskID";
    $stmt = $pdo->prepare($sql);
    ...
    $sql = "UPDATE status SET gold = :gold WHERE userID = :userID";
    $stmt = $pdo->prepare($sql);
    ...
    // 完了したタスクを削除
    $sql = "DELETE FROM UserTasks WHERE userID = :userID AND taskID = :taskID";
    $stmt = $pdo->prepare($sql);
    foreach ($completedTaskIDs as $taskID) {
        $stmt->bindParam(':userID', $userID, PDO::PARAM_INT);
        $stmt->bindParam(':taskID', $taskID, PDO::PARAM_INT);
