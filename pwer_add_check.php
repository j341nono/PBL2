...
try {
    ...
    // ユーザーの現在のhpとpointを取得
    $sql = 'SELECT gold, power, point, powerpoint FROM status WHERE userID = :userID';
    ...
    if ($result) {
        // gold -100, power +1, point -1, powerpoint +1
        if($result['gold']<100){
        ...
        $newGold = $result['gold'] - 100;
        $newPower = $result['power'] + 1;
        $newPoint = $result['point'] - 1;
        $newPowerPoint = $result['powerpoint'] + 1;

        // 更新処理
        $sql = 'UPDATE status SET gold = :gold, power = :power, point = :point, powerpoint = :powerpoint WHERE userID = :userID';
        $stmt = $dbh->prepare($sql);
